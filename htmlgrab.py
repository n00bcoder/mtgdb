...
def getCardInfo(card_id_start, card_id_end):		#M14 set is 370577 - 370825
	cardID = card_id_start
	config = {
		'user': 'carduser',
		'passwd': 'cardpassword',
		'host': 'localhost',
	...
	while cardID <= card_id_end:
		strCardID = str(cardID)
		print "Gathering card ID " + strCardID + "..."
		...
		try:
			...
			cur.close()
		print "Card ID " + strCardID + " done."
		cardID += 1
	print "All done!" #"Number of rows added = ", total_rows <- this only returned 1
	cur.close()
	cnx.close()

print "Run getCardInfo() and pass in the starting card ID number and the ending card ID number you want to get info for."